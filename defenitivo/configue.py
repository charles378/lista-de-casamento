...
def onfique(page: ft.Page):
    ...
    data_falta = ft.Container( 
        content=ft.Row(alignment=ft.MainAxisAlignment.CENTER,vertical_alignment=ft.CrossAxisAlignment.CENTER,controls=[ 
            event_date_input, 
            save_date_button 
                ] 
            ),
        ...
        content=ft.Column(
            controls=[
                ...
                ft.Row(
                    alignment=ft.MainAxisAlignment.END,
                    controls=[
                        ft.IconButton(icon=ft.icons.ADD_A_PHOTO, on_click=lambda _: file_picker.pick_files(allow_multiple=True), icon_color=ft.colors.AMBER_100),
                        ft.IconButton(icon=ft.icons.DELETE, on_click=delete_image, icon_color=ft.colors.RED)
                    ]
                )
            ]
